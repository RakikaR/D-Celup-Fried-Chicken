...
CREATE TABLE public.inventory (
  ...
  terpakai numeric(12,2) GENERATED ALWAYS AS (stok_awal + stok_masuk - stok_akhir) STORED,
  tanggal date NOT NULL DEFAULT CURRENT_DATE,
  created_by uuid REFERENCES auth.users(id),
  UNIQUE (outlet_id, nama_bahan, tanggal)
);
...
CREATE POLICY "Only HQ admin can modify outlets"
...
CREATE POLICY "HQ admin can read all profiles"
...
CREATE POLICY "HQ admin can read all roles"
...
CREATE POLICY "Only HQ admin can modify products"
  ON public.products FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'hq_admin'));

-- --- INVENTORY ---
-- HQ admin: semua. Staff outlet: hanya outlet sendiri.
CREATE POLICY "HQ admin can access all inventory"
  ON public.inventory FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'hq_admin'));

CREATE POLICY "Staff can access own outlet inventory"
  ON public.inventory FOR ALL TO authenticated
  USING (outlet_id = public.get_my_outlet());

-- --- SALES_REPORTS ---
CREATE POLICY "HQ admin can access all sales"
  ON public.sales_reports FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'hq_admin'));

CREATE POLICY "Staff can access own outlet sales"
  ON public.sales_reports FOR ALL TO authenticated
  USING (outlet_id = public.get_my_outlet());
...
INSERT INTO public.outlets (id, nama_outlet, lokasi) VALUES
  ('outlet-1', 'Outlet 1 - Sudirman', 'Jl. Sudirman No. 12, Jakarta'),
  ('outlet-2', 'Outlet 2 - Ahmad Yani', 'Jl. Ahmad Yani No. 45, Bandung'),
  ('outlet-3', 'Outlet 3 - Diponegoro', 'Jl. Diponegoro No. 78, Surabaya');

INSERT INTO public.products (nama_produk, harga, kategori) VALUES
  ('Paket 2 Pcs Ayam + Nasi', 25000, 'Paket'),
  ('Paket 4 Pcs Ayam + Nasi (2)', 45000, 'Paket'),
  ('Paket 6 Pcs Ayam + Nasi (3)', 65000, 'Paket'),
  ('Ayam Crispy 1 Pcs', 13000, 'Ayam'),
  ('Nasi Putih', 5000, 'Side'),
  ('Minuman Soda', 8000, 'Minuman'),
